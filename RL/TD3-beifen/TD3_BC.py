...
class Actor(nn.Module):
	def __init__(self, state_dim, action_dim, max_action):
		...
		self.GRUmodel = Transformer(hidden_size = self.hidden_size+1 ) # GRUModel(hidden_size = self.hidden_size ) # Transformer(action_size = action_size) #  DQN(state_size, action_size) #
...
class Critic(nn.Module):
	def __init__(self, state_dim, action_dim):
		...
		self.hidden_size = 128 
		self.GRUmodel = Transformer(hidden_size = self.hidden_size+1 ) #GRUModel(hidden_size = self.hidden_size )
...
class TD3(object):
	def __init__(
		...
		self.memory_size = int(5e4) # 500 #
		self.memory = deque(maxlen=self.memory_size)
		self.start_timesteps = 2 #5e3
		self.expl_noise = 0.1
		self.batch_size = 256 #128 # 64 #128 #1280 #256
		self.device = device
		self.alpha = 2.5
