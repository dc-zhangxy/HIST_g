        ...
        for yeardatelist in yearly_groups:
            ...
            for dt_ind in range(train_end_index):
                ...
                with torch.no_grad():
                    ...
                    # if (state.isnan().any()):
                    #     print('T')
                    action,_ = agent.take_action(state, deterministic=True, with_logprob=True)  #(np.array(state))
                    xt, reward = env.step(action, label)
                    revenue_list.append(reward)
                    revenue += reward
                if prefix=='Test':
                    preds_df = pd.DataFrame({ 'action': action.item(), 'label': label.cpu().detach().numpy(), 'datetime': st_date ,'instrument':st})
            ...
            # def train_epoch(epoch, optimizer, model, env, train_loader, train_ilens, writer, args):
                ...
                for st in train_ilens['instrument'].drop_duplicates():
                    ...
                    for dt_ind in range(train_end_index):
                        ...
                        state = torch.concat([feature, xt],-1) 
                        # Select action
                        action, _ = model.take_action(state, deterministic=False, with_logprob=True)  #(np.array(state))
                        xt, reward = env.step(action, label)
                        revenue_list.append(reward)
                        revenue += reward
                    ...
                    if len(memory) >= min_size:  # len(agent.memory) >= agent.train_start:
                        model.update(memory)
                    # if steps%10 ==0:
                pprint('evaluating...')
...
def parse_args():
    ...
    parser.add_argument('--early_stop', type=int, default=2500)
    ...
    parser.add_argument('--outdir', default='./output/hq300_SACc_gru_label2to1_logp_pi_a_tanh')
