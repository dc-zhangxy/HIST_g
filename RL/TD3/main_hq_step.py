...
from env_hq_TD3 import HQindexEnv
# from DQN_rec import DQNAgent 
# import TD3s 
# import TD3_BC as TD3   # TD3_BC
import TD3
# import OurDDPG
# import DDPG
    ...
    for i in range(0,len(train_loaders)):
        ...
        for times in range(args.repeat):
            ...
            for epoch in range(args.n_EPISODES):
                ...
                pprint('training...')
                train_sharp = train_epoch(epoch, optimizer, model, env, train_loader, train_ilens, writer, args)
                train_sharp_list.append([epoch,train_sharp])
                pd.DataFrame(train_revenue_list,columns=['epoch','score']).to_pickle(output_path+'/train_sharp_list.pkl')
                # global_step = -1 
                ...
                for st in train_ilens['instrument'].drop_duplicates():
                    ...
                    for dt_ind in range(train_end_index):
                        ...
                        if epoch < args.start_timesteps:
                            ...
                            action = (
                                model.get_action(state)  #(np.array(state))
                                + np.random.normal(0, args.max_action * args.expl_noise, size=args.action_size)
                            ).clip(-args.max_action, args.max_action)
                        ...
                        # if epoch >= args.start_timesteps:  # len(agent.memory) >= agent.train_start:
                        if len(model.memory) >= 600:  # len(agent.memory) >= agent.train_start:
                            model.train_model(epoch)
                            if epoch >= 3 and steps%10 ==0:
                                pprint('evaluating...')
...
def data_prepare(mode, args):
    ...
    test = test[test['STOCK_CODE'] ==300].reset_index(drop=True )
    # test = test[test['STOCK_CODE'].isin(['000852','000905','000300','000016'])].reset_index(drop=True )
    # 如果要跑全市场的，
...
def parse_args():
    ...
    parser.add_argument('--filepath', type=str, default='../data/hq_index5.csv') #'../data/hq_index.csv'
    ...
    parser.add_argument('--outdir', default='./output/hq300_TD3_gru_label2to1_m5w_seed10_step10_noavg')
