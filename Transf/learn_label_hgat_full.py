...
from utils import metric_fn, mse, cal_cos_similarity, cal_convariance
...
def train_epoch(epoch, model, optimizer, train_loader, writer, args):
    ...
    for i, slc in tqdm(train_loader.iter_batch(), total=train_loader.batch_length):
        ...
        if args.model_name == 'FULL_TRANSF':
            ...
            label_re = label.reshape(ba, -1, 6)    # [day, batch, feature]
            # da = label_re.shape[1]
            
            dec_inputs = torch.cat([torch.zeros([1, ba, 6], device = device), label_re.permute(1, 0, 2) ], dim=0)   # Decoder_input 首位为起始符号0 (1
            # dec_outputs = torch.cat([label_re,torch.ones(ba,1,6)], dim=1) # Decoder_output 末位为结束符号1 (2
            pred = model(feature, dec_inputs)[:,1:,:] # 去掉第一天的起始层 (最后一天的结束层已去除)
            loss = loss_fn(pred, label_re, args)
        else:
            pred = model(feature)
            loss = loss_fn(pred, label, args) # + 100*orth_penalty(pred)
...
def orth_penalty(fvs ):
# 两两相关系数平均值，以标准化
    n, f = fvs.size()
    zz = fvs.T.mm(fvs) / (n/(1+1e-9))
    zz_sq = zz.relu().square()
    penalty = (zz_sq.sum() - zz_sq.diag().detach().sum()) /(f*(f-1))
    return penalty 
    ...
    for i, slc in tqdm(test_loader.iter_daily(), desc=prefix, total=test_loader.daily_length):
        ...
        with torch.no_grad():
            if args.model_name == 'FULL_TRANSF':
                ...
                scoress = pd.DataFrame( pred.cpu().numpy(), columns = ['score1','score2','score3','score4'], index=index)
                labelss = pd.DataFrame( label.cpu().numpy(), columns = ['label1','label2','label3','label4'], index=index)
...
def create_loaders(args,train_start_date):
    ...
    hanlder = {'class': 'Alpha180', 'module_path': 'handler.py', 'kwargs': {'start_time': start_time, 'end_time': end_time, 'fit_start_time': start_time, 'fit_end_time': train_end_time, 'instruments': args.data_set, 'infer_processors': [{'class': 'RobustZScoreNorm', 'kwargs': {'fields_group': 'feature', 'clip_outlier': True}}, {'class': 'Fillna', 'kwargs': {'fields_group': 'feature'}}], 'learn_processors': [{'class': 'DropnaLabel'}, {'class': 'CSRankNorm', 'kwargs': {'fields_group': 'label'}}],
                # 'label': ['Ref($close,-6) / Ref($close,-1) - 1','Ref($close,-11) / Ref($close,-1) - 1','Ref($close,-16) / Ref($close,-1) - 1','Ref($close,-21) / Ref($close,-1) - 1']
                # 'label': ['Ref($close,-6) / Ref($close,-1) - 1','Ref($close,-11) / Ref($close,-6) - 1','Ref($close,-16) / Ref($close,-11) - 1','Ref($close,-21) / Ref($close,-16) - 1']
                # }}
    ...
    for i in range(0,len(train_loaders)):
        ...
        for times in range(args.repeat):
            ...
            #     model = get_model(args.model_name)(args.d_feat, args.hidden_size, args.num_layers, args.dropout, 'LSTM')
                ...
                model = get_model(args.model_name)(device=device)  #(d_model=6, nhead=6, num_encoder_layers=6, num_decoder_layers=6, batch_first=True)
            else:
...
def parse_args():
    ...
    # model
    parser.add_argument('--model_name', default='FULL_TRANSF')  # TRANSFORMER  FULL_TRANSF
    ...
    parser.add_argument('--batch_size', type=int, default=512) # -1 indicate daily batch
    ...
    parser.add_argument('--outdir', default='./output/csi300_transfull_for')
