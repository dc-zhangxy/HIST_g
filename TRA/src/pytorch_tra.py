...
class TRAModel(Model):
    ...
    def fvs_scale(self, fvs):
        # 因子标准化
        fvs = fvs - fvs.mean(dim=0, keepdim=True)
        fvs = fvs / (fvs.mul(fvs).mean(dim=0,keepdim=True)+self.eps).sqrt()
        return fvs

    def orth_penalty(self, fvs ):
    # 两两相关系数平均值，以标准化
        n, f = fvs.size()
        zz = fvs.T.mm(fvs) / (n/(1+self.eps))
    ...
    def train_epoch(self, epoch, data_set, is_pretrain=False):
        ...
        for batch in tqdm(data_set, total=max_steps):
            ...
            if is_pretrain or self.transport_method != "none":
                ...
                # print('all_preds.size():' , all_preds.size()) #  torch.Size([1024])
                loss = loss - lamb * reg + self.orth_penalty(all_preds)
            else:
                pred = all_preds.mean(dim=1)
                loss = loss_fn(pred, label) + self.orth_penalty(all_preds) # + all_preds.corr().square().mean()*0.5
