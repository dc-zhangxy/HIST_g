...
file_path = 'output/dq_mv_800_drop_label2to1-vwap_gru_mse/rolling2007-01-01/pred.pkl.test0'
...
pred11 =  pred1.reset_index() # pred1.reset_index() #   pd.merge(pred1.reset_index(), pred2.reset_index()) # 
pred11['hold'] = 1
pred11['signscore'] = (pred11['score1'] > 0.00) # np.sign
...
scores = 'normscorer'  #'score1' # 'normscore'  # 'normscorer'
labels = 'label2'
yuzhi = 0.0
...
if dengquan == True:
    ...
    top_800_stocks[scores] = df_all[scores].apply(lambda x: 0 if x <= yuzhi else 1)
...
# js_df_m = daily_scores_sum_top_m.dropna().iloc[1:]
# bodonglv_m = js_df_m['Ref'].std()* np.sqrt(244)
# # bodonglv 波动率
# nhsyl_m = (js_df_m.iloc[-1]['TIR'])*(244./len(js_df_m)) 
# # nhsyl 年化收益率
# sharpe_m = nhsyl_m/bodonglv_m # 
# previos_max_m =  js_df_m['TIR'].cummax() # 计算上一个最高点
# drawdown_cumsum_m  = js_df_m['TIR']-previos_max_m
# zuidachuiche_m = drawdown_cumsum_m.min()
# zdch_time_m = str(js_df_m['datetime'][np.argmin(zuidachuiche_m)])[:10]
# # 双边换手
# # 全满仓
# df_hs_full = top_800_stocks.reset_index()
# df_hs_full = pd.merge(df_hs_full, df_hs_full.groupby(['datetime'])[scores].sum().reset_index().rename(columns={scores: 'scores_sum'}) )
# df_hs_full['full_cangwei'] = df_hs_full[scores]/df_hs_full['scores_sum']
# df_hs_full['prev_score'] = df_hs_full.groupby('instrument')['full_cangwei'].shift(1).fillna(0)
# df_hs_full['buy_amount'] = (df_hs_full['full_cangwei'] - df_hs_full['prev_score']).clip(lower=0)
# df_hs_full['sell_amount'] =  (df_hs_full['prev_score'] - df_hs_full['full_cangwei']).clip(lower=0)
# df_agg_full = df_hs_full.groupby(['datetime']).agg({'buy_amount':'sum', 'sell_amount':'sum' }) # 所有股票总换手 # 年化
# huanshou_full = df_agg_full.sum()*(244/len(df_agg_full)) 


js_df = daily_scores_sum_top.dropna() #.iloc[1:]
bodonglv = js_df['Ref'].std()* np.sqrt(244)
# bodonglv 波动率
nhsyl = (js_df.iloc[-1]['TIR'])*(244./len(js_df)) 
# nhsyl 年化收益率
...
huanshou = df_cw_agg.sum()*(244/len(df_cw_agg))
...
df_hs_agg['ret'] = df_hs_agg['Ref'] - 15e-4 * df_hs_agg['buy_amount'] - 5e-4 * df_hs_agg['sell_amount']
df_hs_agg['ret2'] = df_hs_agg['Ref'] - 25e-4 * df_hs_agg['buy_amount'] - 15e-4 * df_hs_agg['sell_amount']

df_hs_agg['cum_ret'] = df_hs_agg['ret'].cumsum()
df_hs_agg['cum_ret2'] = df_hs_agg['ret2'].cumsum()

nhsyl_afterfee = (df_hs_agg.iloc[-1]['cum_ret'] )*244./len(js_df)
bodonglv_afterfee = df_hs_agg['ret'].std()*np.sqrt(244)
...
nhsyl_afterfee2 = (df_hs_agg.iloc[-1]['cum_ret2'] )*244./len(js_df)
bodonglv_afterfee2 = df_hs_agg['ret2'].std()*np.sqrt(244)
...
ax1.plot(df_hs_agg.index, df_hs_agg['cum_ret'], label = 'long after fee 20bp', color = 'black' )
ax1.plot(df_hs_agg.index, df_hs_agg['cum_ret2'], label = 'long after fee 40bp', color = 'yellow' )
