...
def create_loaders(args,train_start_date):
    ...
    if args.data_set == 'all':
        ...
        train_end_time = start_time + relativedelta(years=3) - datetime.timedelta(days=1)
        train_end_date = train_end_time.strftime('%Y-%m-%d')

        valid_start_time = start_time + relativedelta(years=3)
        valid_end_time = valid_start_time + relativedelta(years=1) - datetime.timedelta(days=1)
        valid_start_date = valid_start_time.strftime('%Y-%m-%d')
        valid_end_date = valid_end_time.strftime('%Y-%m-%d')

        train_start_time = valid_end_time + datetime.timedelta(days=1)
        end_time = train_start_time + relativedelta(years=1) - datetime.timedelta(days=1)
        test_start_date = train_start_time.strftime('%Y-%m-%d')
        test_end_date = end_time.strftime('%Y-%m-%d')
    else:
        start_time = datetime.datetime.strptime(train_start_date, '%Y-%m-%d')
        train_end_time = start_time + relativedelta(years=4) - datetime.timedelta(days=1)
        train_end_date = train_end_time.strftime('%Y-%m-%d')

        valid_start_time = start_time + relativedelta(years=4)
        valid_end_time = valid_start_time + relativedelta(years=1) - datetime.timedelta(days=1)
        valid_start_date = valid_start_time.strftime('%Y-%m-%d')
        valid_end_date = valid_end_time.strftime('%Y-%m-%d')
        ...
        test_start_date = train_start_time.strftime('%Y-%m-%d')
        test_end_date = end_time.strftime('%Y-%m-%d')
    print(train_start_date,train_end_date,valid_start_date,valid_end_date,test_start_date,test_end_date)
    ...
    start_index = 0
    slc = slice(pd.Timestamp(train_start_date), pd.Timestamp(train_end_date))
    ...
    slc = slice(pd.Timestamp(valid_start_date), pd.Timestamp(valid_end_date))
    ...
    slc = slice(pd.Timestamp(test_start_date), pd.Timestamp(test_end_date))
...
#         stock2concept_matrix = torch.Tensor(stock2concept_matrix).to(device)
    for i in range(0,len(train_loaders)):
        ...
        start_time = datetime.datetime.strptime(start_dates[i], '%Y-%m-%d')
        if args.data_set == 'all':
            test_start_time = start_time + relativedelta(years=4)
        else:
            test_start_time = start_time + relativedelta(years=5)
        test_start_date = test_start_time.strftime('%Y-%m-%d')
        output_path = out_put_path_base +'/rolling' + test_start_date
        if not os.path.exists(output_path):
            os.makedirs(output_path)
        all_precision = []
        all_recall = []
...
def parse_args():
    ...
    parser.add_argument('--train_end_date', default='2016-12-30')
    parser.add_argument('--valid_start_date', default='2017-01-01')
    parser.add_argument('--valid_end_date', default='2017-12-30')
    parser.add_argument('--test_start_date', default='2018-01-01')
    parser.add_argument('--test_end_date', default='2018-12-31')
