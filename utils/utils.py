...
def metric_fn(preds):
    ...
    precision = {}
    recall = {}
    temp = preds.groupby(level=['Date','Time']).apply(lambda x: x.sort_values(by='score', ascending=False))
    if len(temp.index[0]) > 2:
        temp = temp.reset_index(level =[0,1]).drop(['Date','Time'], axis = 1)
        
    for k in [1, 3, 5, 10, 20, 30, 50, 100]:
        precision[k] = temp.groupby(level=['Date','Time']).apply(lambda x:(x.label[:k]>0).sum()/k).mean()
        recall[k] = temp.groupby(level=['Date','Time']).apply(lambda x:(x.label[:k]>0).sum()/(x.label>0).sum()).mean()

    ic = preds.groupby(level=['Date','Time']).apply(lambda x: x.label.corr(x.score)).mean()
    rank_ic = preds.groupby(level=['Date','Time']).apply(lambda x: x.label.corr(x.score, method='spearman')).mean()
