...
# from model_delpre import HIST_DelPre
...
def train_epoch(epoch, model, optimizer, train_loader, writer, args):
    ...
    for i, slc in tqdm(train_loader.iter_batch(), total=train_loader.batch_length):
        ...
        if args.model_name == 'HIST':
            pred = model(feature, None, None ) #stock2concept_matrix[stock_index], market_value
        else:
            pred = model(feature)
...
def create_loaders(args):
    ...
    train_loader = DataLoader(df_train["feature"], df_train["label"], None, None, batch_size=args.batch_size, pin_memory=args.pin_memory, start_index=start_index, device = device)
    ...
    valid_loader = DataLoader(df_valid["feature"], df_valid["label"], None, None, pin_memory=True, start_index=start_index, device = device)
    
    # slc = slice(pd.Timestamp(args.test_start_date), pd.Timestamp(args.test_end_date))
    start_index += len(df_test.groupby(level=0).size())

    test_loader = DataLoader(df_test["feature"], df_test["label"], None, None, pin_memory=True, start_index=start_index, device = device)

    return train_loader, valid_loader, test_loader

def create_loader_800():
    # data_set='csi800'
    # data_dir = 'data/'

    df_train_new = feather.read_dataframe('task_8_30_IDT.feather')

    df_mul = df_train_new.set_index(['Date','Time','Instrument'], drop=True)
    df_mul = df_mul.sort_index(level=['Date','Time'])

    # df_train_new.rename(columns={'END_DATE':'datetime'}, inplace = True)
    # df_mul = df_train_new.set_index(['datetime','STOCK_CODE'], drop=True)
    
    total_time = df_train_new['Date'].drop_duplicates().to_list() # 70天
    # time_index = [str(t) for t in total_time]
    ...
    # train_end_date = time_index[train_end_index] #'2020-12-29'  # 1185
    # valid_start_date = time_index[train_end_index+1] #'2021-01-02'
    # valid_end_date = time_index[valid_end_index] # '2021-12-28' # 1428
    # test_start_date = time_index[valid_end_index+1] #'2022-01-02' # 
    # test_end_date = time_index[-1] # '2022-12-29'

    # transfer = StandardScaler()
    # array_feature = transfer.fit_transform(np.array(df_mul.drop('industry', axis=1).reset_index(drop=True)))

    df_features = df_mul.apply(lambda x:(x-x.mean() )/x.std() )
    # df_features = pd.DataFrame(array_feature, index = df_mul.index, columns=df_mul.columns.to_list()[:2]+df_mul.columns.to_list()[3:])

    df_train = df_features.loc[total_time[:train_end_index]]
    df_valid = df_features.loc[total_time[train_end_index:valid_end_index]]
    df_test = df_features.loc[total_time[valid_end_index:]]
    
    df_feature_train = df_train[df_train.columns[:-2]]
    df_feature_valid = df_valid[df_valid.columns[:-2]]
    df_feature_test = df_test[df_test.columns[:-2]]
    
    label_train = df_train[['volume_label1']] #df_label.loc[time_index[:train_end_index]]
    label_valid = df_valid[['volume_label1']]
    label_test = df_test[['volume_label1']]

    start_index = 0
    train_loader = DataLoader(df_feature_train, label_train, batch_size=args.batch_size, pin_memory=args.pin_memory, start_index=start_index, device = device)

    start_index += len(df_valid.groupby(level=0).size())
    valid_loader = DataLoader(df_feature_valid, label_valid, pin_memory=False, start_index=start_index, device = device)
   
    start_index += len(df_test.groupby(level=0).size())
    test_loader = DataLoader(df_feature_test, label_test, pin_memory=False, start_index=start_index, device = device)
...
def main(args):
    ...
    train_loader, valid_loader, test_loader = create_loader_800()
    ...
    for times in range(args.repeat):
        ...
        for epoch in range(args.n_epochs):
            ...
            # torch.save(model.state_dict(), output_path+'/model_e.bin')
            # torch.save(optimizer.state_dict(), output_path+'/optimizer_e.bin')
...
def parse_args():
    ...
    parser.add_argument('--d_feat', type=int, default=8)
    ...
    parser.add_argument('--name', type=str, default='all_GRU')

    parser.add_argument('--outdir', default='./output/all_GRU_feat8_new')
