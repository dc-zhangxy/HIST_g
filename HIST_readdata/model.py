...
class HIST(nn.Module):
    ...
    def forward(self, x, concept_matrix, market_value):
        # x 的排列方式是 [close_day_lag0, ...,close_day_lag29, ..., vwap_day_lag0, ..., vwap_day_lag29],
        # 与原先 qlib 的区别为 0-29 和 29-0 
        # 因此将 x 全部倒序即可
        x = torch.flip(x, dims=[1]) 
        
        device = torch.device(torch.get_device(x))
        x_hidden = x.reshape(len(x), self.d_feat, -1) # [N, F, T]      # [batch_size, feature_num, time_lens]
